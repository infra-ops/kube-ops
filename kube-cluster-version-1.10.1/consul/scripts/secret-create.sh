...
kubectl create secret generic consul --from-literal="FZvqPeUaa6JuOIiCMV+ibw==" -n kube-system

kubectl create secret generic consul --from-file=/etc/consul/server.json -n kube-system

kubectl create configmap consul --from-file=/etc/consul/server.json -n kube-system


kubectl create secret generic consul --from-literal="gossip-encryption-key=FZvqPeUaa6JuOIiCMV+ibw==" -n kube-system
...
kubectl create secret generic consul \
  ...
  --from-file=consul-key.pem  -n kube-system
